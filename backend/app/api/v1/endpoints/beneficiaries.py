...
def delete_beneficiary(
    ...
    # Invariants check
    has_assistance = db.query(Assistance).filter(Assistance.beneficiary_id == ben.id).first()
    if has_assistance:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot delete beneficiary with existing financial assistance or loan records. Please deactivate instead."
        )
    ...
    AuditService.log(
        ...
        entity_name="beneficiaries",
        entity_id=str(ben.id),
        old_values={"name": ben.name, "beneficiary_code": ben.beneficiary_code, "group_id": str(ben.group_id)},
        user_id=current_user.id,
        ip_address=get_client_ip(request)
    )
    db.delete(ben)
    db.commit()
    return {"message": f"Beneficiary '{ben.name}' deleted successfully."}
